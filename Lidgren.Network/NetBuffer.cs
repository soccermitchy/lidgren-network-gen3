...
namespace Lidgren.Network
{
	public partial class NetBuffer
	{
		...
		static NetBuffer()
		{
			...
			{
				if (mi.GetParameters().Length == 0 && mi.Name.StartsWith("Read", StringComparison.CurrentCulture) && mi.Name.Substring(4) == mi.ReturnType.Name)
				{
			...
			foreach (MethodInfo mi in methods)
			{
				if (mi.Name.Equals("Write", StringComparison.CurrentCulture))
				{
